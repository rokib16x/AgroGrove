...
if (isset($_POST['submit'])) {
    ...
    if ($password == $cpassword) {
        $sql = "SELECT * FROM instructor WHERE ins_email ='$email' OR address='$address1'";

        $result = mysqli_query($conn, $sql);
        if (!$result->num_rows > 0 ) {
            $sql = "INSERT INTO instructor (first_name, last_name, ins_email, mobile_no, Address,password)
            ...
            echo "<script>alert('Woops! Email or Address Already Exists.')</script>";
        }
    } 
    else 
    {
...
<body>
    <div class="container">
        <form action="" method="POST" class="login-email">
            ...
            <p class="login-register-text">Have an account? <a href="inslogin.php">Login Here</a></p>
        </form>
    </div>
</body>
