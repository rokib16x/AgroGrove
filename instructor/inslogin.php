...
if (isset($_POST['submit'])) {
    ...
    $sql = "SELECT * FROM instructor WHERE ins_email='$email' AND password='$password'";

    $result = mysqli_query($conn, $sql);
    if ($result->num_rows > 0) {
        $row = mysqli_fetch_assoc($result);
        $_SESSION['tempmail'] = $row['ins_email'];
        header("Location: instructordashboard.php");
    } else {
...
<body>
    <div class="container">
        <form action="" method="POST" class="login-email">
            ...
            <p class="login-register-text">Don't have an account? <a href="insregister.php">Register Here</a></p>
