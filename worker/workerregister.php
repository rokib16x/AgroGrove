...
if (isset($_POST['submit'])) {
    ...
    if ($password == $cpassword) {
        $sql = "SELECT * FROM worker WHERE w_email ='$email'";
        $result = mysqli_query($conn, $sql);
        if (!$result->num_rows > 0) {
            $sql = "INSERT INTO worker (first_name, last_name, w_email, mobile_no, Address, minsalary, password)
					VALUES ('$firstname', '$lastname','$email','$mobileno','$address1', '$salary','$password')";
            $result = mysqli_query($conn, $sql);
            if ($result) {
...
<body>
    <div class="container">
        <form action="" method="POST" class="login-email">
            ...
            <p class="login-register-text">Have an account? <a href="workerlogin.php">Login Here</a></p>
        </form>
    </div>
</body>
